...
function processTextForHighlighting(text: string, highlightColor: string) {
	...
	// Get first match before entering the loop to avoid assignment in the condition
...
// Props type definition for the OGImageEvent component
type OGImageEventProps = {
	...
	event?: Event | null // Optional event data to display
...
// Main component for generating an Open Graph image for events
export default function OGImageEvent({
	title,
	size,
	secondary,
	organizer,
	...
	const titleMain = String(title ?? 'Découvrez nos **événements** sportifs.')
	const secondaryDesc = String(secondary ?? "Trouvez l'**événement** parfait pour votre prochain défi sportif")
