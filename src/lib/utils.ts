...
/**
 * Get a random participant count based on event type and distance 🏃‍♀️
 */
export const getRandomParticipantCount = (eventType: string, distance: number): number => {
	...
	switch (eventType) {
		...
		case 'running':
			...
			break
		case 'trail':
			baseCount = Math.floor(500 + Math.random() * 2000) // 500-2500 🏞️
			break
		case 'triathlon':
	...
	if (distance > 100) {
		...
		baseCount = Math.floor(baseCount * 0.6) // Long distances 🏃‍♂️
	} else if (distance > 20) {
		baseCount = Math.floor(baseCount * 0.8) // Medium distances 🏃‍♀️
	}
...
const EVENT_TEMPLATES = [
	...
	{
		type: 'triathlon',
		name: 'Triathlon de {city}',
		distance: 51,
		basePrice: 80,
	},
	{
		type: 'cycle',
		name: 'Cyclosportive de {city}',
		distance: 120,
		basePrice: 40,
	},
] as const
...
/**
 * Generate a complete BibSale object with realistic faker data 🛍️
 */
