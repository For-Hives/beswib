...
export default async function Image({ params }: { params: Promise<LocaleParams> }) {
	...
	const titleRaw = tOg.events?.OG?.titleOG ?? tOgEn.events?.OG?.titleOG
	const titleLines = (titleRaw ?? 'Cherchez de nouveaux\n dossards et évènements.')
		.replace(/<br\s*\/?>/gi, '\n')
		.split('\n')
		.map(line => line.trim())
		.filter(Boolean)

	const secondaryRaw = tOg.events?.OG?.descriptionOG ?? tOgEn.events?.OG?.descriptionOG ?? ''
