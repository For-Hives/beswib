...
export async function generateArticleTranslationAction(
	...
	try {
		...
		// Add Gemini's internal logs
		if (translatedContent.logs && translatedContent.logs.length > 0) {
			logs.push(...translatedContent.logs)
		}

		logs.push('✓ Translation generated by Gemini AI')
		logs.push(`✓ Generated slug: "${translatedContent.slug}"`)

		// Create or update image with translated alt text
		...
		if (sourceArticle.expand?.image?.image) {
			...
			if (existingTranslation?.image) {
				...
				const { updateImageWithAltById } = await import('@/services/article.services')
				await updateImageWithAltById(existingTranslation.image, translatedContent.imageAlt)
				imageId = existingTranslation.image
				logs.push(`✓ Image alt text updated: ${existingTranslation.image}`)
				logs.push(`✓ New alt text: "${translatedContent.imageAlt}"`)
			} else {
				...
				if (newImage) {
					imageId = newImage.id
					logs.push(`✓ Image created: ${newImage.id}`)
					logs.push(`✓ Image alt text: "${translatedContent.imageAlt}"`)
				}
			}
		}

		// Create or update SEO record with translated metadata
		logs.push('🔍 Processing SEO metadata...')
		let seoId = ''
		if (existingTranslation?.seo) {
			// Update existing SEO record
			const { updateSEOById } = await import('@/services/seo.services')
			const seoResult = await updateSEOById(existingTranslation.seo, {
				title: translatedContent.seoTitle,
				description: translatedContent.seoDescription,
			})
			if (seoResult) {
				seoId = seoResult.id
				logs.push(`✓ SEO record updated: ${seoResult.id}`)
				logs.push(`✓ SEO title: "${translatedContent.seoTitle}"`)
			}
		} else {
			...
			if (seoResult) {
				seoId = seoResult.id
				logs.push(`✓ SEO record created: ${seoResult.id}`)
				logs.push(`✓ SEO title: "${translatedContent.seoTitle}"`)
			}
		}
		...
		if (existingTranslation) {
			...
			if (result) {
				logs.push(`✓ Article created: ${result.id}`)
			}
		}
