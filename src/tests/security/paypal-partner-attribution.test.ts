...
import { createOrder, capturePayment, getMerchantId, listPayPalWebhooks, setupPayPalWebhooks, getMerchantIntegrationStatus } from '@/services/paypal.services'
...
describe('PayPal Partner Attribution Header', () => {
	beforeEach(() => {
		...
		mockedFetch.mockImplementation((url) => {
			...
			return Promise.resolve(
				new Response(JSON.stringify({ id: 'test-response', status: 'success' }), {
					status: 200,
					headers: { 'Content-Type': 'application/json' },
				})
			)
	...
	describe('createOrder', () => {
		it('should include PayPal-Partner-Attribution-Id header', async () => {
			const mockBib = {
				id: 'bib-123',
				price: 50.0,
				user: { id: 'user-123', firstName: '', lastName: '' },
			...
			const createOrderCall = mockedFetch.mock.calls.find(call => 
				typeof call[0] === 'string' && call[0].includes('/v2/checkout/orders') && !call[0].includes('/capture')
			)
	...
	describe('capturePayment', () => {
		it('should include PayPal-Partner-Attribution-Id header', async () => {
			...
			const captureCall = mockedFetch.mock.calls.find(call => 
				typeof call[0] === 'string' && call[0].includes('/capture')
			)
	...
	describe('getMerchantId', () => {
		it('should include PayPal-Partner-Attribution-Id header', async () => {
			...
			const getMerchantCall = mockedFetch.mock.calls.find(call => 
				typeof call[0] === 'string' && call[0].includes('/partner-referrals/')
			)
	...
	describe('listPayPalWebhooks', () => {
		it('should include PayPal-Partner-Attribution-Id header', async () => {
			...
			const listWebhooksCall = mockedFetch.mock.calls.find(call => 
				typeof call[0] === 'string' && call[0].includes('/notifications/webhooks') &&
				call[1]?.method === 'GET'
			)
	...
	describe('setupPayPalWebhooks', () => {
		it('should include PayPal-Partner-Attribution-Id header', async () => {
			...
			const setupWebhooksCall = mockedFetch.mock.calls.find(call => 
				typeof call[0] === 'string' && call[0].includes('/notifications/webhooks') &&
				call[1]?.method === 'POST'
			)
	...
	describe('getMerchantIntegrationStatus', () => {
		it('should include PayPal-Partner-Attribution-Id header', async () => {
			...
			const statusCall = mockedFetch.mock.calls.find(call => 
				typeof call[0] === 'string' && call[0].includes('/merchant-integrations/')
			)
